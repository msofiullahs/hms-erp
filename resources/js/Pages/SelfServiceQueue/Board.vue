...
<template>
    <Head title="Papan Antrian" />

    <div class="min-h-screen bg-slate-950 px-8 py-8 text-white">
        <header class="flex items-center justify-between">
            <div>
                <p class="text-sm uppercase tracking-widest text-cyan-400">Papan Antrian</p>
                <h1 class="mt-1 text-3xl font-semibold">Sistem Antrian Rumah Sakit</h1>
            </div>
            <div class="text-right text-xs text-slate-400">
                <p>Auto-refresh setiap {{ props.refreshSeconds }} detik</p>
                <p v-if="updatedAt" class="mt-1 font-mono">{{ formatTime(updatedAt) }}</p>
                <p v-if="error" class="mt-1 text-red-400">Koneksi terganggu: {{ error }}</p>
            </div>
        </header>
        ...
        <section class="mt-8 grid gap-6 lg:grid-cols-3">
            <div class="lg:col-span-2 rounded-3xl border border-cyan-500/40 bg-gradient-to-br from-slate-900 to-slate-800 p-10 text-center">
                <p class="text-sm uppercase tracking-widest text-cyan-300">Sedang Dilayani</p>
                <template v-if="nowServing">
                    ...
                    <p class="mt-2 text-lg text-cyan-300">Silakan menuju Loket {{ nowServing.counter || '—' }}</p>
                </template>
                <template v-else>
                    <p class="mt-16 text-2xl text-slate-500">Belum ada antrian yang dipanggil.</p>
                </template>
            </div>

            <div class="rounded-3xl border border-slate-800 bg-slate-900 p-6">
                <p class="text-sm uppercase tracking-widest text-slate-400">Dipanggil Sebelumnya</p>
                <ul class="mt-4 space-y-3">
                    ...
                    >
                        ...
                        <span class="rounded-xl bg-slate-700 px-3 py-1 text-xs">Loket {{ ticket.counter || '—' }}</span>
                    </li>
        ...
        <section class="mt-8 rounded-3xl border border-slate-800 bg-slate-900 p-6">
            <div class="flex items-center justify-between">
                <p class="text-sm uppercase tracking-widest text-slate-400">Sedang Menunggu</p>
                <span class="rounded-xl bg-slate-800 px-3 py-1 text-xs text-slate-300">{{ waiting.length }} tiket</span>
            </div>
